...
abs_folder_path = Path(r'E:\1_TTM闪烁体\TTM自由基X衍射成像\文章\文章数据\PSF_TTM3PCz_TSCTAu2_absPL_CHCl3\PSF_Au_TTM3PCz_abs_CHCl3') # 输入要处理的吸收光谱文件所在文件夹的路径
pl_folder_path = Path(r'E:\1_TTM闪烁体\TTM自由基X衍射成像\文章\文章数据\PSF_TTM3PCz_TSCTAu2_absPL_CHCl3\PSF_Au_TTM3PCz_PL_CHCl3') # 发射光谱所在的文件夹路径
...
for i,file in enumerate(df_abs_files):
    ...
    x = abs_data.iloc[:,0]
    y = abs_data.iloc[:,1]
    ax1.plot(x,y,label = file.stem[3:],linestyle = '--',linewidth = '3',color = colors[i])
